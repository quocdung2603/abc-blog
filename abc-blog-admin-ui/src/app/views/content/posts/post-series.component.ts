...
export class PostSeriesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.buildForm();
    //Load data to form
    let series = this.seriesApiClient.getAllSeries();
    ...
    forkJoin({
      ...
      .subscribe({
        next: (repsonse: any) => {
          ...
          let series = repsonse.series as SeriesInListDto[];
  ...
  removeSeries(id: string) {
    let body: AddPostSeriesRequest = new AddPostSeriesRequest({
      postId: this.config.data.id,
      seriesId: id,
    });
    this.seriesApiClient
      .deletePostToSeries(body)
      .pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: () => {
